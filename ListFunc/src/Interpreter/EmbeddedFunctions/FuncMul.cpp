...
std::unique_ptr<Expression> FuncMul::operator()(const std::vector<const Expression*>& args) const {
	auto arg1 = args[0]->evaluate();
	auto arg2 = args[1]->evaluate();

	if (arg1->type() != ExpressionType::RealNumber ||
		arg2->type() != ExpressionType::RealNumber) {
		throw std::invalid_argument("mul takes 2 real numbers as arguments");
	}
